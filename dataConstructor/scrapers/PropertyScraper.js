...
const flatsPromise2 = axios.get('https://www.rettie.co.uk/all-property-for-sale/sales/tag-apartment/up-to-750000/page-2?page_size=100')

const properties = []
let id = 0

Promise.all([housesPromise, flatsPromise, flatsPromise2])
    .then(([housesHtml, flatsHtml, flats2Html]) => {
        dataConstructor(housesHtml.data, "House")
        dataConstructor(flatsHtml.data, "Flat")
        dataConstructor(flats2Html.data, "Flat")
        return properties
    })
    .then(res => {
        ...
        fs.writeFile('jsonStore/properties.json', propertiesJSON, function (err) {
            ...
            console.log('Properties Complete');
          });
...
dataConstructor = (html, category) => {
    ...
    console.log(`Propteries(${category}) Scraping Finished`);
